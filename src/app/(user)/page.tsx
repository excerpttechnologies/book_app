...
export default function HomePage() {
  ...
  return (
    <div>
      {/* Hero Slider */}
      <section style={{ position: 'relative', height: 480, overflow: 'hidden', background: 'linear-gradient(135deg, var(--accent-dark) 0%, #1F3A52 50%, var(--gold) 100%)' }}>
        {currentSlide?.image
        ...
        <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.35)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ textAlign: 'center', color: '#fff', padding: '0 20px', maxWidth: 700 }}>
            ...
            <div style={{ display: 'flex', gap: 14, justifyContent: 'center', flexWrap: 'wrap' }}>
              <Link href="/shop" className="btn-primary" style={{ background: '#fff', color: '#1F3A52', padding: '12px 28px', fontSize: '1rem' }}>
                Shop Now
              </Link>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 16 }}>
          {CATEGORIES.map(cat => (
            <Link key={cat.name} href={`/category/${cat.slug}`}
              ...
              onMouseEnter={e => { (e.currentTarget.style.borderColor = '#1F3A52'); (e.currentTarget.style.transform = 'translateY(-3px)'); (e.currentTarget.style.boxShadow = 'var(--shadow-md)'); }}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 20 }}>
            {LANGUAGES.map((lang, i) => (
              <Link key={lang.name} href={`/shop?language=${lang.name}`}
                style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '32px 20px', borderRadius: 'var(--radius)', background: ['#1F3A52', 'var(--gold)', 'var(--green)'][i], color: '#fff', textAlign: 'center', transition: 'transform 0.2s, box-shadow 0.2s' }}
      ...
      {bestSellers.length > 0 && (
        <section style={{ maxWidth: 1280, margin: '0 auto', padding: '60px 20px 40px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28 }}>
            ...
            <Link href="/shop?bestSeller=true" style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 14, color: '#1F3A52', fontWeight: 500 }}>
        ...
        <section style={{ background: 'var(--bg-secondary)', padding: '60px 20px' }}>
          <div style={{ maxWidth: 1280, margin: '0 auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28 }}>
              ...
              <Link href="/shop?sort=createdAt&order=desc" style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 14, color: '#1F3A52', fontWeight: 500 }}>
                View All <FiArrowRight size={16} />
              </Link>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 20 }}>
